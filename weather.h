...
void fillWeatherFromJson(Weather* weather) {
  ...
  sprintf(weather->tempH1, "%2i\xb0", (int) round(json["hourly"][1]["temp"]));
  sprintf(weather->feelsLikeH1, "%2i\xb0", (int) round(json["hourly"][1]["feels_like"]));
  sprintf(weather->humidityH1, "%3i %%", (int) json["hourly"][1]["humidity"]);

  sprintf(weather->iconD, "%s", (const char*) json["daily"][0]["weather"][0]["icon"]);
  sprintf(weather->tempMinD, "%2i\xb0", (int) round(json["daily"][0]["temp"]["min"]));
  sprintf(weather->tempMaxD, "%2i\xb0", (int) round(json["daily"][0]["temp"]["max"]));
  sprintf(weather->humidityD, "%3i %%", (int) json["daily"][0]["humidity"]);

  sprintf(weather->iconD1, "%s", (const char*) json["daily"][1]["weather"][0]["icon"]);
  sprintf(weather->tempMinD1, "%2i\xb0", (int) round(json["daily"][1]["temp"]["min"]));
  sprintf(weather->tempMaxD1, "%2i\xb0", (int) round(json["daily"][1]["temp"]["max"]));
